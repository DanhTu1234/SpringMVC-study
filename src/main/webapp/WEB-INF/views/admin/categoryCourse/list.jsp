...
<c:url var="courseCategoryAPI" value="/api/category"/>
...
<form action="<c:url value="/quan-tri/danh-muc/danh-sach"/>" id="formSubmit" method="get">
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                ...
                <a href="<c:url value='/quan-tri/danh-muc/them-moi' />" class="btn btn-primary btn-icon-split btn-sm">
                    ...
                    <table class="table table-bordered table-hover text-center align-middle shadow-sm" style="background-color:#fff">
                        ...
                        <c:forEach var = "item" items = "${model.listResult}" varStatus="status">
                            <tr>
                                ...
                                <td>${item.name}</td>
                                <td>
                                    <a class="btn btn-sm btn-outline-primary" href="<c:url value='/quan-tri/danh-muc/chinh-sua'>
    ...
    $(document).ready(function () {
        $('.btn-delete').click(function () {
            ...
            if (confirm('Bạn có chắc muốn xóa khoa này?')) {
                $.ajax({
                    url: '${courseCategoryAPI}/'+ id, // API xóa
                    method: 'DELETE',
